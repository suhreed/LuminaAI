      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-5">
        ...
        <div className="bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-2xl p-4 space-y-3">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-[var(--text-primary)] font-bold text-base leading-tight">{book.title}</h2>
              <p className="text-xs text-[var(--text-muted)] mt-0.5">{book.genre} · {book.targetLanguage}</p>
              <p className="text-xs text-[var(--text-secondary)] mt-2 leading-relaxed line-clamp-3">{book.topic}</p>
            </div>
          </div>
          ...
          {totalChapters > 0 && (
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-xs">
                <span className="text-[var(--text-muted)]">Writing Progress</span>
                <span className="text-[var(--text-secondary)] font-medium">{writtenChapters.length}/{totalChapters} chapters</span>
              </div>
        ...
        <div className="space-y-3">
          ...
          <button
            onClick={onGoOutline}
            className="w-full flex items-center gap-4 bg-[var(--bg-secondary)] border border-[var(--border-color)] hover:border-violet-700/50 hover:bg-[var(--bg-tertiary)]/50 rounded-xl px-4 py-4 transition-colors text-left"
          >
          ...
          <button
            ...
            className="w-full flex items-center gap-4 bg-[var(--bg-secondary)] border border-[var(--border-color)] hover:border-violet-700/50 hover:bg-[var(--bg-tertiary)]/50 disabled:opacity-40 disabled:cursor-not-allowed rounded-xl px-4 py-4 transition-colors text-left"
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[var(--text-primary)] font-medium text-sm">Write Manuscript</p>
              <p className="text-xs text-[var(--text-muted)] mt-0.5">
        ...
        {book.outlineApproved && book.chapters.length > 0 && (
          <div className="space-y-3">
            <h3 className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider">Chapters</h3>
              ...
              {[...book.chapters]
                ...
                .map((ch) => (
                  <button
                    ...
                    className="w-full flex items-center gap-3 bg-[var(--bg-secondary)] border border-[var(--border-color)] hover:bg-[var(--bg-tertiary)]/50 rounded-xl px-4 py-3 transition-colors text-left"
                  >
                    <span className="text-xs font-bold text-violet-400 w-5 flex-shrink-0">
                      {ch.chapterNumber}
                    </span>
                    <span className="flex-1 text-sm text-[var(--text-secondary)] truncate">{ch.title}</span>
                    {ch.content.trim() ? (
                      <CheckCircle size={14} className="text-emerald-500 flex-shrink-0" />
                    ) : (
                      <Sparkles size={14} className="text-[var(--border-color)] flex-shrink-0" />
                    )}
                  </button>
                ))}
            </div>
