...
export default function ManuscriptWriter({ book, apiKey, initialChapter, onBack, onUpdate }: Props) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[var(--bg-primary)]">
      {/* Header */}
      <header className="flex items-center gap-2 px-4 py-3 border-b border-[var(--border-color)] flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-[var(--text-muted)]">{writtenCount}/{totalChapters} chapters written</p>
        </div>
        {/* Export */}
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-[var(--text-muted)] hover:text-[var(--text-primary)] disabled:text-[var(--border-color)] transition-colors px-2 py-1.5 rounded-lg hover:bg-[var(--bg-tertiary)]"
          >
            {isExporting ? <Loader2 size={14} className="animate-spin" /> : <FileDown size={14} />}
            Export
          </button>
          {showExportMenu && (
            <div className="absolute right-0 top-full mt-1 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl overflow-hidden shadow-xl z-10 min-w-[140px]">
              <button
                onClick={handleExportMarkdown}
                className="w-full flex items-center gap-2 px-4 py-3 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)] transition-colors"
              >
                <FileText size={14} />
                Markdown (.md)
              </button>
      ...
      {/* Chapter Navigation */}
      <div className="flex items-center gap-2 px-4 py-3 border-b border-[var(--border-color)] flex-shrink-0 overflow-x-auto">
        ...
        {currentChapter ? (
          <div className="space-y-4">
            ...
            <div>
              ...
              <h2 className="text-lg font-bold text-[var(--text-primary)]">{currentChapter.title}</h2>
              {outlineChapter && (
                <p className="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">{outlineChapter.summary}</p>
              )}
            </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 disabled:bg-[var(--bg-tertiary)] disabled:text-[var(--text-muted)] text-white text-sm font-medium py-2.5 rounded-xl transition-colors"
              >
                {isGenerating ? (
              ...
              {!editMode && (
                <button
                  ...
                  className="flex items-center gap-1.5 bg-[var(--bg-tertiary)] hover:bg-[var(--border-color)] text-[var(--text-secondary)] text-sm font-medium px-3 py-2.5 rounded-xl transition-colors"
                >
                  <Edit3 size={15} />
            ...
            {editMode ? (
              <div className="space-y-3">
                <textarea
                  ...
                  className="w-full bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl px-4 py-3 text-[var(--text-primary)] text-sm leading-relaxed focus:outline-none focus:border-violet-500 resize-none min-h-[400px]"
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-[var(--bg-tertiary)] hover:bg-[var(--border-color)] text-[var(--text-secondary)] text-sm font-medium px-4 py-2.5 rounded-xl transition-colors"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl">
                <Edit3 size={32} className="text-[var(--border-color)] mb-3" />
                <p className="text-[var(--text-muted)] text-sm">This chapter hasn&apos;t been written yet</p>
                <p className="text-[var(--text-muted)] text-xs mt-1">Use AI to generate it or write it manually</p>
              </div>
            )}
          </div>
        ) : (
          <div className="flex items-center justify-center py-20">
            <p className="text-[var(--text-muted)]">No chapter selected</p>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-t border-[var(--border-color)] flex-shrink-0">
        <button
          ...
          className="flex items-center gap-1.5 text-sm text-[var(--text-muted)] hover:text-[var(--text-primary)] disabled:text-[var(--border-color)] transition-colors px-3 py-2 rounded-lg hover:bg-[var(--bg-tertiary)] disabled:hover:bg-transparent"
        >
